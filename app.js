...
function fillExSel(sessions) {
  ...
  sel.innerHTML = sortedExs.map(label => {
    let display = label;
...
function drawProgress() {
  const exLabel = document.getElementById('exSel').value;
    ...
    s.exercises.forEach(ex => {
      ...
      ex.sets.forEach(set => {
        if (isOt) {
          ...
          if (km > 0 && (!(label in prKm) || km > prKm[label])) prKm[label] = km;
        } else {
          const w = parseFloat(set.weight) || 0;
          if (w > 0 && (!(label in prKg) || w > prKg[label])) prKg[label] = w;
        }
      });
    });
  });
